...
class Custom(KolibriPluginBase):
    translated_view_urls = "urls"


@register_hook
class CustomNavItem(NavigationHook):
    bundle_id = "side_nav"


@register_hook
class CustomAsset(webpack_hooks.WebpackBundleHook):
    bundle_id = "app"
...
@register_hook
class CustomContentNodeHook(ContentNodeDisplayHook):
    def node_url(self, node):
        ...
        if kind_slug is not None:
            return (
                reverse("kolibri:kolibri.plugins.custom:custom")
                + "#/topics/"
                + kind_slug
                + node.id
            )
